...
- [ ] implement Serialization for config, for more reliable integration tests
  - [ ] make a macro to generate config file in fs to then deserialize (also ensures two-way 'se & 'de).
